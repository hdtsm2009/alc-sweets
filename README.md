...
## 商品開発担当者向けガイド（2026-09-12）

`/about/`を商品開発の実務ガイドへ改稿。企画条件→参考を探す→比較・判断→試作メモ→会議・次の試作の順に、各工程で残す内容と既存機能への導線を案内する。試作メモ15項目に対応する架空の企画例、素材・構成・価格・製造負荷の比較軸、保存済み参照記録と最新情報の扱い、1端末1案の保存・書き出しを説明。ナビゲーション名は「開発ガイド」。

実行結果：`tests 40 / pass 40 / fail 0`、静的ページ`11/11`、書き出し`2/2`。記入例は`15/15`項目を照合。ブラウザの目視・操作は未確認。詳細は [開発ガイド更新報告](../database/DEVELOPMENT_GUIDE_REPORT.md) を参照。
