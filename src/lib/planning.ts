...
export const COMPARISON_GROUPS: { title: string; fields: [keyof Product, string][] }[] = [
  { title: "最新の確認情報（元DBの履歴と区別）", fields: [["informationCheckedAt", "公式情報確認日"], ["latestSourceDate", "公式発表日"], ["latestUpdateSummary", "更新内容"], ["latestDescription", "最新の商品説明"], ["latestPrice", "最新掲載価格（原文）"], ["latestAvailability", "最新の販売予定・取扱い"]] },
...
export const DEVELOPMENT_GROUP = COMPARISON_GROUPS.find(group => group.fields.some(([key]) => key === "ALC試作案"))!;
export const LATEST_INFORMATION_GROUP = COMPARISON_GROUPS.find(group => group.fields.some(([key]) => key === "informationCheckedAt"))!;
...
export function draftMarkdown(draft: TartDraft, products: Product[]): string {
  ...
  for (const p of products) {
    ...
    for (const key of ["一次情報URL", "URL", "価格出典URL", "latestInfoUrl"] as const) if (safeHttpUrl(p[key])) lines.push(`${key}: ${p[key]}`);
    if (p.informationCheckedAt) {
      lines.push(`最新の公式情報確認日: ${p.informationCheckedAt}（現在販売・在庫の確認とは別）`);
      for (const [key, label] of LATEST_INFORMATION_GROUP.fields) if (key !== "informationCheckedAt" && p[key]) lines.push(`${label}: ${p[key]}`);
    }
