...
export default function PlanningPage() {
  ...
  const referencesChanged = JSON.stringify(references.map(p => p.商品ID)) !== JSON.stringify(selected.map(p => p.商品ID));
  ...
  return <div>
    <div className="flex flex-wrap justify-between gap-4 items-start mb-6">
      ...
      <div className="flex flex-wrap gap-2 no-print"><a className="action action-primary" href="#draft">試作メモを書く ↓</a><Link className="action" href="/">参考商品を探す</Link></div>
    </div>
    ...
    {ready && selected.length === 0 ? <div className="surface p-6 mb-6"><h2 className="font-bold mb-2">比較候補を選んでください</h2><p className="text-sm text-gray-600">検索・商品詳細の「比較候補に追加」から選べます。下の試作メモは候補なしでも記入できます。</p><Link className="inline-block text-[#1F4E78] underline mt-3" href="/">検索へ</Link></div> : selected.length > 0 && <section className="mb-8">
      ...
      <div className="comparison-wrap" tabIndex={0} aria-label="候補比較表。横にスクロールできます">
        <table className="comparison-table"><thead><tr><th scope="col">比較項目</th>{selected.map(p => <th scope="col" key={p.商品ID}>
          ...
          <Link href={`/product/?id=${encodeURIComponent(p.商品ID)}&from=planning`} className="text-[#1F4E78] underline">{p.商品名}</Link>
          ...
          <tr><th scope="row">出典</th>{selected.map(p => <td key={p.商品ID}>{(["一次情報URL", "URL", "価格出典URL"] as const).map(key => safeHttpUrl(p[key]) ? <a key={key} href={safeHttpUrl(p[key])} target="_blank" rel="noopener noreferrer" className="block text-[#1F4E78] underline mb-2">{key}</a> : null)}{!p.URL && !p.一次情報URL && <span>未記録</span>}</td>)}</tr>
        </tbody></table>
      </div>
    </section>}
    <section id="draft" className="surface p-5 md:p-7 scroll-mt-4">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 mb-5 text-sm">
        ...
        {referencesChanged && <p className="text-amber-800 mt-2">比較中の候補とは異なります。保存・書き出しには、この案に紐づく記録を使います。</p>}
