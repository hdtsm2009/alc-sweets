...
export default function ImagesPage() {
  ...
  if (loading) return <p className="py-20 text-center">画像の取得状況を読み込み中...</p>;
  return <div>
    ...
    <div className="space-y-3">{filtered.map(p => <article className="surface p-4 md:p-5" key={p.商品ID}><div className="flex flex-wrap gap-2 items-center"><span className="text-xs text-slate-500">{p.商品ID} / {p.ブランド名}</span><span className={`rounded px-2 py-1 text-xs ${p.imageKind === "generated" ? "bg-amber-100 text-amber-900" : "bg-slate-100 text-slate-600"}`}>{p.imageDisplayStatus === "matched" ? p.imageKind === "social" ? "SNS掲載写真" : "掲載写真" : p.imageDisplayStatus === "generated" ? "生成イメージ" : "未取得・対応未確認"}</span></div><h2 className="font-bold text-[#1F4E78] mt-2"><Link href={`/product/?id=${encodeURIComponent(p.商品ID)}`}>{p.商品名}</Link></h2><p className="text-sm text-slate-600 leading-relaxed mt-2">{p.imageGenerationBasis || p.imageFailureReason || p.imageMatchNote || "掲載元の代表画像と商品の対応を特定できていません。"}</p><div className="flex flex-wrap gap-x-4 gap-y-1 text-xs mt-3 text-slate-500"><span>調査日：{p.imageSearchDate || p.imageCheckedAt || "未記録"}</span>{safeHttpUrl(p.imageSourceUrl || p.URL) && <a href={safeHttpUrl(p.imageSourceUrl || p.URL)} target="_blank" rel="noopener noreferrer" className="underline">掲載元</a>}<Link className="underline" href={`/product/?id=${encodeURIComponent(p.商品ID)}`}>画像・詳細を見る →</Link></div></article>)}</div>
